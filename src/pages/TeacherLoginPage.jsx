...
const FROM = '#10b981', TO = '#14b8a6', GLOW = 'rgba(16,185,129,0.28)';
...
const TeacherLoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const { data: teacher, error } = await supabase.from('teachers').select('*')
        .ilike('name', form.name).eq('id', parseInt(form.id))
        .eq('school_id', parseInt(schoolId)).single();
      if (error || !teacher) throw new Error();
      saveSession({ userRole:'teacher', userId:teacher.id, userName:teacher.name, schoolId });
      toast({ title:'✓ '+t('loginSuccess'), className:'bg-emerald-500/10 border-emerald-500/50 text-emerald-400' });
  ...
  return (
    <>
      <Helmet><title>{t('role_teacher')} Login · CloudCampus</title></Helmet>
      <Shell from={FROM} to={TO} glow={GLOW} schoolId={schoolId} navigate={navigate} t={t}
        title={t('role_teacher')} subtitle={t('portalTagTeacher')} iconBg="bg-emerald-500/15"
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <Field label={t('fullName')} icon={<User className="h-4 w-4" />}>
            <Input type="text" placeholder={t('namePlaceholder')} required
              className="pl-10 h-12 bg-white/5 border-white/10 focus:border-emerald-500/60 rounded-xl"
              value={form.name} onChange={e=>setForm({...form,name:e.target.value})} />
          </Field>
          <Field label={t('teacherId')} icon={<Lock className="h-4 w-4" />}>
            <Input type="number" placeholder={t('teacherIdPlaceholder')} required
              className="pl-10 h-12 bg-white/5 border-white/10 focus:border-emerald-500/60 rounded-xl"
