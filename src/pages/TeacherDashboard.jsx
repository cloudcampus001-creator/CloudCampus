...
import { Home, Activity, Bell, Inbox, Upload, GraduationCap, LogOut, Menu, Cloud, MessageSquare } from 'lucide-react';
...
const ACCENT = { from: 'from-emerald-500', to: 'to-teal-400', glow: 'shadow-emerald-500/40', text: 'text-emerald-400', ring: 'ring-emerald-500/30', mobileActive: 'bg-emerald-500/15', iconGlow: 'drop-shadow-[0_0_8px_rgba(16,185,129,0.9)]' };
...
const TeacherDashboard = () => {
  ...
  const navItems = [
    { icon: Home,          label: t('overview'),     path: '/dashboard/teacher',               shortLabel: 'Home'    },
    { icon: Activity,      label: t('liveActivity'), path: '/dashboard/teacher/activity',      shortLabel: 'Live'    },
    { icon: Bell,          label: t('notify'),       path: '/dashboard/teacher/notify',        shortLabel: 'Notify'  },
    { icon: Inbox,         label: 'Notifications',   path: '/dashboard/teacher/notifications', shortLabel: 'Inbox'   },
    { icon: Upload,        label: t('publish'),      path: '/dashboard/teacher/publish',       shortLabel: 'Publish' },
    { icon: GraduationCap, label: t('marks'),        path: '/dashboard/teacher/marks',         shortLabel: 'Marks'   },
    { icon: MessageSquare, label: t('chat'),         path: '/dashboard/teacher/chat',          shortLabel: 'Chat'    },
  ];
  const handleSignOut = async () => { await signOut(); localStorage.clear(); navigate('/'); };
  const isActive = (p) => p === '/dashboard/teacher' ? location.pathname === p : location.pathname.startsWith(p);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row font-sans selection:bg-emerald-500 selection:text-white">
      <motion.aside initial={false} animate={{ width: isSidebarOpen ? 272 : 72 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }}
        ...
        <div className="p-5 flex items-center justify-between h-[72px] shrink-0">
          <AnimatePresence>{isSidebarOpen && (<motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.2 }}>
            <LogoDropdown><div className={`flex items-center gap-2 font-bold text-xl bg-clip-text text-transparent bg-gradient-to-r ${ACCENT.from} ${ACCENT.to}`}><Cloud className="text-emerald-500 h-6 w-6" /><span>Cloud<span className="text-foreground">Campus</span></span></div></LogoDropdown>
          </motion.div>)}</AnimatePresence>
          <Button variant="ghost" size="icon" onClick={() => setIsSidebarOpen(o => !o)} className="text-muted-foreground hover:text-emerald-400 shrink-0 hover:bg-emerald-500/10 rounded-xl"><Menu className="h-5 w-5" /></Button>
        </div>
          ...
          <div className={cn('flex items-center gap-2', !isSidebarOpen && 'flex-col')}>
            ...
            {isSidebarOpen ? (
              <button onClick={() => setProfileOpen(true)} className={`ml-auto flex items-center gap-2.5 ring-1 ${ACCENT.ring} rounded-xl px-2.5 py-1.5 hover:bg-white/5 transition-all active:scale-95`}>
                ...
                <div className="text-sm overflow-hidden text-left"><p className="font-semibold leading-none truncate max-w-[90px]">{userName}</p><p className="text-[10px] text-muted-foreground mt-0.5">Teacher</p></div>
              </button>
            ) : (
      ...
      <div className="md:hidden flex items-center justify-between px-4 h-14 border-b border-white/10 bg-card/80 backdrop-blur-xl sticky top-0 z-50">
        <LogoDropdown><div className={`flex items-center gap-2 font-bold text-base bg-clip-text text-transparent bg-gradient-to-r ${ACCENT.from} ${ACCENT.to}`}><Cloud className="text-emerald-500 h-5 w-5" /><span>CloudCampus</span></div></LogoDropdown>
      ...
      <main className="flex-1 p-4 md:p-8 pb-28 md:pb-8 overflow-y-auto h-[calc(100vh-56px)] md:h-screen scroll-smooth">
        <Routes>
          <Route path="/" element={<TeacherHomePage />} /><Route path="/activity" element={<ActivityPage />} />
          <Route path="/notify" element={<NotifyPage />} /><Route path="/publish" element={<PublishPage />} />
          <Route path="/marks" element={<MarksPage />} /><Route path="/chat" element={<TeacherChatPage />} />
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 px-3 pb-3">
        <div className="bg-card/80 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.35)] overflow-hidden">
          <div className="flex items-stretch px-1 py-1 overflow-x-auto scrollbar-none">{navItems.map((item) => { const active = isActive(item.path); return (
            <Link key={item.path} to={item.path} className="flex-1 min-w-[48px]">
      ...
      <ProfileSheet open={profileOpen} onClose={() => setProfileOpen(false)} userName={userName} roleLabel="Teacher" accentFrom={ACCENT.from} accentTo={ACCENT.to} onSignOut={handleSignOut} />
    </div>
  );
};
