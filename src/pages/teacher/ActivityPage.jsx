 ...
 *
 * Validation layers (both must pass before any submission):
 *  1. Time window  — teacher must be in an active timetable period (existing logic)
 *  2. Geolocation  — teacher must be within school.geo_radius_meters of
 *                    school.latitude / school.longitude (new)
 *
 * If the school record has no latitude/longitude configured the geo check
 * is skipped gracefully (GeoStatus = 'not_configured') and the teacher
...
/* ── Geo status banner ────────────────────────────────── */
const GeoStatusBanner = ({ status, distance, radius, onRetry }) => {
  ...
  const cfg = {
    ...
    verified: {
      ...
      text: `Location verified · ${Math.round(distance ?? 0)} m from school`,
      cls:  'bg-emerald-500/10 border-emerald-500/25 text-emerald-400',
    },
    failed: {
      icon: <XCircle className="h-4 w-4" />,
      text: `You are ${Math.round(distance ?? 0)} m from school (max ${radius} m). Move closer to sign the logbook.`,
      cls:  'bg-red-500/10 border-red-500/25 text-red-400',
    },
    denied: {
      icon: <AlertTriangle className="h-4 w-4" />,
      text: 'Location access denied. Enable GPS in your browser to sign the logbook.',
      cls:  'bg-amber-500/10 border-amber-500/25 text-amber-400',
    },
    error: {
      icon: <AlertTriangle className="h-4 w-4" />,
      text: 'Could not obtain your location. Check your GPS and try again.',
      cls:  'bg-amber-500/10 border-amber-500/25 text-amber-400',
    },
  }[status] ?? null;
  ...
  return (
    ...
    >
      ...
      {(status === 'failed' || status === 'denied' || status === 'error') && (
        <button
          onClick={onRetry}
          className="shrink-0 flex items-center gap-1 text-[11px] font-bold opacity-70 hover:opacity-100 transition-opacity"
        >
...
const ActivityPage = () => {
  ...
  /* ── geolocation state ──────────────────────────────── */
  // 'idle' | 'checking' | 'verified' | 'failed' | 'denied' | 'error' | 'not_configured'
  const [geoStatus,   setGeoStatus]   = useState('idle');
  const [geoDistance, setGeoDistance] = useState(null);   // metres
  const [schoolGeo,   setSchoolGeo]   = useState(null);   // { latitude, longitude, geo_radius_meters }
  ...
  /* ── fetch current class + school geo ───────────────── */
  useEffect(() => {
    const init = async () => {
      try {
        setLoading(true);

        /* 1. Fetch active timetable period */
        const now         = new Date();
        ...
        /* 3. Start geo check (only if class found — avoids waking GPS unnecessarily) */
        if (timetable) {
          checkGeo(geo);
        }
      } catch (err) {
  ...
  // True when geo check is required AND currently blocking submission
  const geoBlocking =
    schoolGeo?.latitude &&
  ...
  const handleSubmitLog = async (e) => {
    ...
    if (geoBlocking) {
      toast({
        variant: 'destructive',
        title: t('geoRequired') || 'Location required',
        description: t('geoRequiredDesc') || 'You must be at school to sign the logbook.',
      });
      return;
    }
    ...
    try {
      const { error } = await supabase.from('e_logbook_entries').insert({
        ...
        // store whether geo was verified for audit
  ...
  const handleSubmitAttendance = async () => {
    if (geoBlocking) {
      toast({
        variant: 'destructive',
        title: t('geoRequired') || 'Location required',
        description: t('geoRequiredDesc') || 'You must be at school to submit the register.',
      });
      return;
    }
  ...
  return (
    <>
      <Helmet>
        <title>{t('liveActivityTitle')} — CloudCampus</title>
      </Helmet>
      <PageTransition>
              ...
              >
                <div className="glass rounded-2xl p-6 space-y-5">
                      ...
                      <div className="ml-auto shrink-0">
                        {geoStatus === 'verified' ? (
                          <span className="flex items-center gap-1.5 text-xs font-semibold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-full">
                            <Navigation className="h-3 w-3" /> In range
                          </span>
                        ) : geoStatus === 'checking' ? (
                          <span className="flex items-center gap-1.5 text-xs font-semibold text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2.5 py-1 rounded-full">
                            <Loader2 className="h-3 w-3 animate-spin" /> Locating…
                          </span>
                        ) : geoStatus !== 'not_configured' ? (
                  ...
                  <form onSubmit={handleSubmitLog} className="space-y-4">
                    ...
                    <button
                      type="submit"
                      disabled={submittingLog || geoBlocking}
                      className={cn(
              ...
              >
                <div className="glass rounded-2xl p-6 space-y-5">
                  ...
                  <button
                    ...
                    disabled={submittingAtt || geoBlocking}
                    className={cn(
