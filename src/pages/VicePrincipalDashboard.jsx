...
import { Home, LogOut, Menu, Cloud, BookOpen, Bell, FileCheck, MessageSquare, BookMarked } from 'lucide-react';
...
const getInitials = (name = '') => name.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2) || 'VP';
const ACCENT = { from: 'from-purple-500', to: 'to-pink-500', glow: 'shadow-purple-500/40', text: 'text-purple-400', ring: 'ring-purple-500/30', mobileActive: 'bg-purple-500/15', iconGlow: 'drop-shadow-[0_0_8px_rgba(168,85,247,0.9)]' };
...
const VicePrincipalDashboard = () => {
  ...
  const navItems = [
    ...
    { icon: BookMarked,    label: 'Attr. Subjects',     path: '/dashboard/vice-principal/attribute-subjects',shortLabel: 'Subjects' },
    { icon: Bell,          label: t('notify'),          path: '/dashboard/vice-principal/notify',            shortLabel: 'Notify'   },
    { icon: FileCheck,     label: t('marksheetReview'), path: '/dashboard/vice-principal/marks',             shortLabel: 'Marks'    },
  ...
  useEffect(() => {
    ...
    supabase.from('classes').select('id, name').eq('vp_id', userId).then(({ data }) => {
      if (data?.length) {
        ...
        setSelectedClassId(stored && data.find(c => c.id.toString() === stored) ? stored : data[0].id.toString());
      }
    });
  }, [userId]);
  const handleClassChange = (v) => { setSelectedClassId(v); localStorage.setItem('vpSelectedClass', v); };
  const handleSignOut = async () => { await signOut(); localStorage.clear(); navigate('/'); };
  const isActive = (p) => p === '/dashboard/vice-principal' ? location.pathname === p : location.pathname.startsWith(p);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row font-sans selection:bg-purple-500 selection:text-white">
      <motion.aside initial={false} animate={{ width: isSidebarOpen ? 272 : 72 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }}
        ...
        <div className="p-5 flex items-center justify-between h-[72px] shrink-0">
          <AnimatePresence>{isSidebarOpen && (<motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.2 }}>
            <LogoDropdown><div className={`flex items-center gap-2 font-bold text-xl bg-clip-text text-transparent bg-gradient-to-r ${ACCENT.from} ${ACCENT.to}`}><Cloud className="text-purple-500 h-6 w-6" /><span>Cloud<span className="text-foreground">Campus</span></span></div></LogoDropdown>
          </motion.div>)}</AnimatePresence>
          <Button variant="ghost" size="icon" onClick={() => setIsSidebarOpen(o => !o)} className="text-muted-foreground hover:text-purple-400 shrink-0 hover:bg-purple-500/10 rounded-xl"><Menu className="h-5 w-5" /></Button>
        </div>
          ...
          <div className={cn('flex items-center gap-2', !isSidebarOpen && 'flex-col')}>
            ...
            {isSidebarOpen ? (
              <button onClick={() => setProfileOpen(true)} className={`ml-auto flex items-center gap-2.5 ring-1 ${ACCENT.ring} rounded-xl px-2.5 py-1.5 hover:bg-white/5 transition-all active:scale-95`}>
                ...
                <div className="text-sm overflow-hidden text-left"><p className="font-semibold leading-none truncate max-w-[90px]">{userName}</p><p className="text-[10px] text-muted-foreground mt-0.5">Vice Principal</p></div>
              </button>
            ) : (
      ...
      <div className="md:hidden flex items-center justify-between px-4 h-14 border-b border-white/10 bg-card/80 backdrop-blur-xl sticky top-0 z-50">
        <LogoDropdown><div className={`flex items-center gap-2 font-bold text-base bg-clip-text text-transparent bg-gradient-to-r ${ACCENT.from} ${ACCENT.to}`}><Cloud className="text-purple-500 h-5 w-5" /><span>CloudCampus</span></div></LogoDropdown>
      ...
      <main className="flex-1 p-4 md:p-8 pb-28 md:pb-8 overflow-y-auto h-[calc(100vh-56px)] md:h-screen scroll-smooth flex flex-col">
        {!isChatPage && (
          <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 glass p-4 rounded-xl shrink-0">
            <div><h2 className="text-base font-semibold">{t('dashboardContext')}</h2><p className="text-xs text-muted-foreground">{t('selectClass')}</p></div>
            <div className="w-full sm:w-[250px]">
              <Select value={selectedClassId} onValueChange={handleClassChange} disabled={managedClasses.length === 0}>
                <SelectTrigger className="bg-background/60 border-purple-500/30 focus:ring-purple-500"><SelectValue placeholder={managedClasses.length === 0 ? t('noClasses') : t('selectClass')} /></SelectTrigger>
                <SelectContent>{managedClasses.map(cls => <SelectItem key={cls.id} value={cls.id.toString()}>{cls.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
          </div>
        )}
      ...
      <ProfileSheet open={profileOpen} onClose={() => setProfileOpen(false)} userName={userName} roleLabel="Vice Principal" accentFrom={ACCENT.from} accentTo={ACCENT.to} onSignOut={handleSignOut} />
    </div>
  );
};
