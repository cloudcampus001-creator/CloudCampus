...
import { 
  Home, Users, LogOut, Menu, User, Cloud, CalendarClock, MessageSquare, School, BookMarked
} from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  // Invisible — requests push permission and signals device on new notifications
  useDeviceNotifications();

  const userName = localStorage.getItem('userName') || 'Administrator';

  const navItems = [
    { icon: Home, label: t('overview'), path: '/dashboard/administrator' },
    { icon: Users, label: t('users'), path: '/dashboard/administrator/users' },
    { icon: School, label: 'Classes', path: '/dashboard/administrator/classes' },
    { icon: BookMarked, label: 'Subjects & Library', path: '/dashboard/administrator/subjects' },
    { icon: CalendarClock, label: t('timetables'), path: '/dashboard/administrator/timetables' },
    { icon: MessageSquare, label: t('systemChat'), path: '/dashboard/administrator/chat' },
  ];
  ...
  const isPathActive = (path) => {
    if (path === '/dashboard/administrator' && location.pathname === '/dashboard/administrator') return true;
    if (path !== '/dashboard/administrator' && location.pathname.startsWith(path)) return true;
    return false;
  };
        ...
        <div className="p-6 flex items-center justify-between">
          {isSidebarOpen && (
             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
               <LogoDropdown>
                 <div className="flex items-center gap-2 font-bold text-xl bg-clip-text text-transparent bg-gradient-to-r from-indigo-500 to-violet-500">
                   <Cloud className="text-indigo-500 h-6 w-6" />
                   <span>Cloud<span className="text-foreground">Campus</span></span>
                 </div>
               </LogoDropdown>
             </motion.div>
          )}
          <Button variant="ghost" size="icon" onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="text-muted-foreground hover:text-indigo-500">
          ...
          <div className={cn("flex items-center justify-between", !isSidebarOpen && "flex-col gap-4")}>
             ...
             {isSidebarOpen && (
                 <div className="flex items-center gap-3">
                    <div className="h-8 w-8 rounded-full bg-gradient-to-tr from-indigo-500 to-violet-500 flex items-center justify-center text-white font-bold">
      ...
      <div className="md:hidden flex items-center justify-between p-4 border-b border-white/10 bg-card/80 backdrop-blur-xl sticky top-0 z-50">
        <LogoDropdown>
          <div className="flex items-center gap-2 font-bold text-lg">
            <Cloud className="text-indigo-500 h-5 w-5" />
        ...
        {navItems.map((item) => (
          <Link key={item.path} to={item.path} className="flex-1">
            <div className={cn(
                ...
                isPathActive(item.path) ? "text-indigo-500 scale-110" : "text-muted-foreground"
              )}>
