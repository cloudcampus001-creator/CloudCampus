...
import { Bell, School, User, Info, Paperclip, ExternalLink } from 'lucide-react';
...
const formatTime = (d) => {
  const date = new Date(d), diff = new Date() - date;
...
const groupByDay = (items) => {
  const map = {};
  items.forEach(n => { const k = formatDay(n.created_at); if (!map[k]) map[k] = []; map[k].push(n); });
  return Object.entries(map);
};

const TYPE_STYLE = {
  school:          { icon: School, dot: 'bg-indigo-500', ring: 'ring-indigo-500/30', label: 'School-wide', cls: 'bg-indigo-500/15 text-indigo-400 border-indigo-500/25' },
  vice_principal:  { icon: User,   dot: 'bg-pink-500',   ring: 'ring-pink-500/30',   label: 'Personal',    cls: 'bg-pink-500/15  text-pink-400  border-pink-500/25'   },
  default:         { icon: Info,   dot: 'bg-purple-500', ring: 'ring-purple-500/30', label: 'General',     cls: 'bg-purple-500/15 text-purple-400 border-purple-500/25' },
};
...
const VPNotificationsPage = () => {
  ...
  useEffect(() => {
    if (!schoolId) return;
    const ch = supabase.channel(`vp_notifs_${schoolId}_${userId}`)
      .on('postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'notifications', filter: `school_id=eq.${parseInt(schoolId)}` },
        ({ new: n }) => {
          const ok = n.target_type === 'school' || (n.target_type === 'vice_principal' && String(n.target_id) === String(userId));
          if (ok) setNotifs(p => [n, ...p]);
        }
      ).subscribe();
  ...
  const personalCount = notifs.filter(n => n.target_type === 'vice_principal').length;

  const FILTERS = [
    { id: 'all',           label: t('all'),       count: notifs.length },
    { id: 'school',        label: 'School-wide',  count: schoolCount,   hidden: schoolCount === 0 },
    { id: 'vice_principal',label: 'Personal',     count: personalCount, hidden: personalCount === 0 },
  ].filter(f => !f.hidden);
  ...
  return (
    <>
      <Helmet><title>{t('notifications')} · VP · CloudCampus</title></Helmet>
      <PageTransition>
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-sm text-muted-foreground mt-1">{t('vpNotifsDesc')}</p>
          </motion.div>
              ...
              {FILTERS.map(f => (
                <button key={f.id} onClick={() => setFilter(f.id)}
                  className={cn(
                    ...
                    filter === f.id
                      ? 'bg-gradient-to-r from-purple-500 to-pink-500 text-white border-transparent shadow-md shadow-purple-500/25'
          ...
          {loading && <div className="space-y-3">{[0,1,2,3,4].map(i => <NotifSkeleton key={i} />)}</div>}
                  ...
                  return (
                    <motion.div key={n.id} variants={fadeUp}>
                      <button onClick={() => setSelected(n)}
                        ...
                        <div className="glass rounded-2xl p-4 border border-white/8 hover:border-purple-500/25 hover:bg-white/4 transition-all duration-200 active:scale-[0.99]">
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-start gap-3 flex-1 min-w-0">
                              <div className={cn('p-2 rounded-xl shrink-0 mt-0.5', `${st.cls.split(' ')[0]}`)}>
                                <st.icon className={cn('h-4 w-4', st.cls.split(' ').slice(1, 2).join(' '))} />
                              </div>
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-3 mt-2 text-[10px] text-muted-foreground">
                                  {n.sender_name && <span>{t('notifFrom')}: <span className="font-semibold">{n.sender_name}</span></span>}
                                  {n.file_url    && <span className="flex items-center gap-0.5 text-purple-400"><Paperclip className="h-2.5 w-2.5" />Attachment</span>}
                                </div>
                              </div>
                            </div>
      ...
      <GlassPopup
        ...
        subtitle={selected ? `${t('notifFrom')}: ${selected.sender_name || '—'} · ${new Date(selected.created_at).toLocaleDateString(undefined, { day: 'numeric', month: 'long', year: 'numeric' })}` : ''}
        variant="sheet" maxWidth="max-w-md"
        footer={selected?.file_url ? (
          <a href={selected.file_url} target="_blank" rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 w-full py-3.5 rounded-2xl bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold shadow-lg shadow-purple-500/25 transition-all active:scale-[0.98]">
      ...
      >
        {selected && (
          <div className="space-y-4">
            <span className={cn('text-xs font-bold px-3 py-1.5 rounded-full border', typeStyle(selected).cls)}>
...
export default VPNotificationsPage;
