...
const ParentLoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const { data: s, error } = await supabase.from('students').select('*')
        .ilike('name', form.studentName).eq('matricule', form.matricule)
        .eq('school_id', parseInt(schoolId)).single();
      if (error || !s) throw new Error();
      saveSession({ userRole:'parent', userId:s.matricule, userName:'Parent of '+s.name, schoolId, classId:s.class_id||'', studentName:s.name });
      localStorage.setItem('studentMatricule', s.matricule);
      toast({ title:'✓ '+t('loginSuccess'), className:'bg-blue-500/10 border-blue-500/50 text-blue-400' });
      navigate('/dashboard/parent');
    } catch { toast({ variant:'destructive', title:t('loginFailed'), description:t('invalidStudentDetails') }); }
