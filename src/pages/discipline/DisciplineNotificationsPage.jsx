 ...
 *
 * Definitive version — structurally identical to TeacherNotificationsPage
 * and VPNotificationsPage: same date helpers, same groupByDay, same
 * TYPE_STYLE → typeStyle pattern, same clickable timeline, same GlassPopup
 * detail sheet, same realtime subscription.
 *
 * Orange / red brand accent throughout.
 * Marks all notifications as read on mount (same key DisciplineDashboard uses).
 */
...
/* ── date helpers (identical to Teacher / VP) ─────────── */
const formatDay = (d) => {
...
/* ── type → style (same structure as Teacher / VP) ───── */
const TYPE_STYLE = {
  school: {
    icon:  Bell,
    dot:   'bg-orange-500',
...
/* ── animation presets (identical to Teacher / VP) ───── */
...
const DisciplineNotificationsPage = () => {
  ...
  /* ── fetch (same pattern as Teacher / VP) ─────────── */
  useEffect(() => {
    if (!schoolId) return;
    (async () => {
      setLoading(true);
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      `notif_read_at_discipline_${schoolId}_${userId}`,
      new Date().toISOString()
    );
  }, [schoolId, userId]);

  /* ── realtime (same pattern as Teacher / VP) ──────── */
  useEffect(() => {
    if (!schoolId) return;
    const ch = supabase
  ...
  /* ── filter counts (same pattern as Teacher / VP) ── */
  ...
  /* ── render (structurally identical to VP / Teacher) */
  return (
    <>
      <Helmet>
      ...
      <PageTransition>
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          {/* Timeline (identical structure to VP / Teacher) */}
            ...
            <motion.div key={day} variants={stagger} initial="hidden" animate="visible" className="space-y-2">
              ...
              <div className="relative ml-5 space-y-3">
                ...
                {items.map(n => {
                  ...
                  const [bgCls, textCls] = st.cls.split(' ');
                  return (
                    <motion.div key={n.id} variants={fadeUp}>
                      <button onClick={() => setSelected(n)} className="relative w-full text-left pl-8 group">
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-start gap-3 flex-1 min-w-0">
                              ...
                              <div className={cn('p-2 rounded-xl shrink-0 mt-0.5', bgCls)}>
                                <st.icon className={cn('h-4 w-4', textCls)} />
                              </div>
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-3 mt-2 text-[10px] text-muted-foreground">
                                  {n.sender_name && (
                                    <span>
                                      {t('notifFrom')}: <span className="font-semibold capitalize">{n.sender_name}</span>
                                    </span>
                                  )}
                                  {n.file_url && (
      ...
      {/* Detail GlassPopup (identical to VP / Teacher) */}
      <GlassPopup
        open={!!selected}
        ...
        subtitle={
          selected
            ? `${t('notifFrom')}: ${selected.sender_name || '—'} · ${new Date(selected.created_at).toLocaleDateString(undefined, { day: 'numeric', month: 'long', year: 'numeric' })}`
            : ''
        }
        variant="sheet"
        maxWidth="max-w-md"
