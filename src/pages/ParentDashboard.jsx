...
import { Home, FileText, Shield, BookOpen, MessageSquare, LogOut, Menu, Cloud, Bell } from 'lucide-react';
...
const ParentDashboard = () => {
  ...
  /* Fetch class name for the profile sheet */
  useEffect(() => {
    if (!classId) return;
  ...
  const navItems = [
    { icon: Home,          label: t('overview'),   path: '/dashboard/parent',               shortLabel: 'Home'      },
    { icon: Bell,          label: t('notify'),     path: '/dashboard/parent/notifications', shortLabel: 'Alerts'    },
    { icon: FileText,      label: t('documents'),  path: '/dashboard/parent/docs',          shortLabel: 'Docs'      },
    { icon: Shield,        label: t('discipline'), path: '/dashboard/parent/discipline',    shortLabel: 'Discipline' },
    { icon: BookOpen,      label: t('library'),    path: '/dashboard/parent/library',       shortLabel: 'Library'   },
    { icon: MessageSquare, label: t('chat'),       path: '/dashboard/parent/chat',          shortLabel: 'Chat'      },
  ];

  const handleSignOut = async () => { await signOut(); localStorage.clear(); navigate('/'); };
  const isActive = (p) => p === '/dashboard/parent' ? location.pathname === p : location.pathname.startsWith(p);

  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row font-sans selection:bg-blue-500 selection:text-white"
      style={{ '--primary': '221 83% 53%', '--primary-foreground': '210 40% 98%', '--ring': '221 83% 53%' }}>

      {/* ── Desktop Sidebar ─────────────────────────────────── */}
      <motion.aside initial={false} animate={{ width: isSidebarOpen ? 272 : 72 }}
            ...
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.2 }}>
              <LogoDropdown>
                <div className={`flex items-center gap-2 font-bold text-xl bg-clip-text text-transparent bg-gradient-to-r ${ACCENT.from} ${ACCENT.to}`}>
                  <Cloud className="text-blue-500 h-6 w-6" /><span>Cloud<span className="text-foreground">Campus</span></span>
                </div>
              </LogoDropdown>
            </motion.div>
          )}</AnimatePresence>
          ...
          <nav className="space-y-1">{navItems.map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path} className="relative group block">
                <div className={cn('flex items-center rounded-xl transition-all duration-200 relative overflow-visible',
                  isSidebarOpen ? 'px-3 py-2.5' : 'px-0 py-2.5 justify-center',
                  active ? `bg-gradient-to-r ${ACCENT.from} ${ACCENT.to} text-white shadow-lg ${ACCENT.glow}` : 'text-muted-foreground hover:text-foreground hover:bg-white/5')}>
        ...
        <div className="p-4 border-t border-white/5 space-y-3 shrink-0">
          <div className={cn('flex items-center gap-2', !isSidebarOpen && 'flex-col')}>
            ...
            {isSidebarOpen ? (
              <button onClick={() => setProfileOpen(true)}
                ...
                <div className={`h-7 w-7 rounded-full bg-gradient-to-tr ${ACCENT.from} ${ACCENT.to} flex items-center justify-center text-white text-xs font-bold shrink-0`}>{getInitials(studentName)}</div>
                <div className="text-sm overflow-hidden text-left">
                  <p className="font-semibold leading-none truncate max-w-[90px]">{studentName}</p>
                  <p className="text-[10px] text-muted-foreground mt-0.5 truncate max-w-[90px]">{studentClassName || 'Parent'}</p>
                </div>
              </button>
            ) : (
              <button onClick={() => setProfileOpen(true)}
          ...
          <Button onClick={handleSignOut} className={cn('w-full bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 rounded-xl', isSidebarOpen ? 'justify-start' : 'justify-center px-2')}>
            <LogOut className={cn('h-4 w-4', isSidebarOpen && 'mr-2')} />{isSidebarOpen && <span className="text-sm">{t('logout')}</span>}
          </Button>
        </div>
      </motion.aside>
      ...
      <div className="md:hidden flex items-center justify-between px-4 h-14 border-b border-white/10 bg-card/80 backdrop-blur-xl sticky top-0 z-50">
        <LogoDropdown>
          <div className={`flex items-center gap-2 font-bold text-base bg-clip-text text-transparent bg-gradient-to-r ${ACCENT.from} ${ACCENT.to}`}>
            <Cloud className="text-blue-500 h-5 w-5" /><span>CloudCampus</span>
          </div>
        </LogoDropdown>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 px-3 pb-3">
        <div className="bg-card/80 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.35)] overflow-hidden">
          <div className="flex items-stretch px-1 py-1">{navItems.map((item) => {
            const active = isActive(item.path);
            return (
      ...
      {/* ── iOS Profile Sheet (Parent special) ──────────────── */}
      <ProfileSheet
        open={profileOpen} onClose={() => setProfileOpen(false)}
        userName={userName} roleLabel="Parent"
